...
let repeatedWord = require('./hashmap-repeated-word.js');

describe('Repeated Word Test', () => {
  it('happy path', ()=>{
    let words='Once upon a time, there was a brave princess who...';
  ...
  it('should raise an exception if there is no repeated words', ()=>{
    expect(repeatedWord('to live life freely')).toBe('no repeated words!');
  });
  it('should raise an exception if there is no repeated words', ()=>{
    expect(repeatedWord('to live life freely')).toBe('no repeated words!');
  });
  it('should raise an exception if not passing a parameter', ()=>{
    expect(repeatedWord()).toEqual('please pass a parameter');
  });
